...
| `amplitude` | the amplitude of the inhomogeneous GSN solution at infinity and at the horizon such that $X^{\infty}_{\ell m \omega} = $ amplitude_inf $ e^{i \omega r_*}$ and $X^{\mathrm{H}}_{\ell m \omega} = $ amplitude_hor $ e^{- i \kappa r_*}$|
| `energy_flux` | the energy flux emitted towards infinity and down horizon|
| `angular_momentum_flux` | the angular momentum flux towards infinity and down horizon|
| `Carter_const_flux` | the Carter constant flux towards infinity and down horizon|
...
| :--- | :--- |
| `mode` | a [Mode](@ref) object storing information about the mode (including the frequency) |
| `amplitude` | the amplitude of the inhomogeneous Teukolsky solution at infinity and at the horizon such that $R^{\infty}_{\ell m \omega} = $ amplitude_inf $ r^{-(2s+1)} e^{i \omega r_*}$ and $R^{\mathrm{H}}_{\ell m \omega} = $ amplitude_hor $ \Delta^{-s} e^{- i \kappa r_*}$|
| `energy_flux` | the energy flux emitted towards infinity and down horizon|
| `angular_momentum_flux` | the angular momentum flux towards infinity and down horizon|
| `Carter_const_flux` | the Carter constant flux towards infinity and down horizon|
| `trajectory` | the trajectory/geodesic that the particle follows |
