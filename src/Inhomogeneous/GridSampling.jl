...
export swsh_sample, y_sample
export integrand_generic_sample, integrand_eccentric_sample, integrand_inclined_sample
export kerr_geo_generic_sample_cheby, kerr_geo_eccentric_sample_cheby, kerr_geo_inclined_sample_cheby
export integrand_generic_sample_cheby, integrand_eccentric_sample_cheby, integrand_inclined_sample_cheby
...
function y_sample(Y_soln, KG_sample::Dict)
    a = Y_soln.mode.a
...
function integrand_generic_sample(KG_samp, Y_samp, SH_samp, n::Int64, k::Int64)
    # --------------------------
    ...
    return 4im .* π .* ω .* Jlmnk .* exp(im * Xi) ./ Γ
end

function integrand_generic_sample_cheby(KG_samp, Y_samp, SH_samp, n::Int64, k::Int64)
    # --------------------------
    # 1. Extract Parameters (1D Vectors)
    # --------------------------
    # Radial (N) and Polar (K) sampling vectors
...
function integrand_eccentric_sample(KG_samp, Y_samp, SH, n::Int64)
    ...
    # --------------------------
    sinθ = sin(θ)                 # sin(π/2) = 1
    cosθ = cos(θ)                 # cos(π/2) = 0
    cotθ = cot(θ)                 # cot(π/2) = 0
    inv_sinθ = 1.0 / sinθ         # 1/sin(π/2) = 1
    inv_sin2θ = 1.0 / (sinθ ^ 2)  # 1/sin²(π/2) = 1
    ...
    return 4im .* π .* ω .* Jlmn .* exp.(im .* Xi) ./ Γ
end

function integrand_eccentric_sample_cheby(KG_samp, Y_samp, SH, n::Int64)
    # --------------------------
    # 1. Extract Parameters (1D Radial, θ as constant)
    # --------------------------
    r_vec = KG_samp["r"]          # Radial sampling points (length N)
...
function integrand_inclined_sample(KG_samp, Y_dic, SH_samp, k::Int64)
    # --------------------------
    ...
    return 4im .* π .* ω .* Jlmk .* exp.(im .* Xi) ./ Γ
end

function integrand_inclined_sample_cheby(KG_samp, Y_dic, SH_samp, k::Int64)
    # --------------------------
    # 1. Extract Parameters (1D Polar, r fixed as constant)
    # --------------------------
    r = KG_samp["r"]              # Fixed radial coordinate (scalar)
