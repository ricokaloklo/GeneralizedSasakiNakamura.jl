...
function iterate_inf_in(xm, s, epsilon, tau, kappa, lambda, N = _TruncatioN, l = nothing, tol = _TOLERANCE, sfe = 0)
    use_sfe = sfe == 1
    if use_sfe
        nu_in, coeffs_inf, Psfe_in, Rin = sfe_in(s, epsilon, tau, kappa, lambda, N, l)
        x_min = - Rin
    ...
    while x_min < xm
        coeffs_ord = ordinary_point_coeffs(s, epsilon, tau, kappa, lambda, x_min, P, P_prime, N)
        Δx = min(convergence_radius_x0_right(x_min, s, epsilon, tau, kappa, lambda, coeffs_ord, tol), convergence_radius_x0_left(x_min, s, epsilon, tau, kappa, lambda, coeffs_ord, tol))
        if !(isfinite(Δx)) || Δx <= 0
            @error("Step size Δx invalid: $Δx")
        end
    ...
    function evaluate_P(x)
        ...
        elseif x_list[end] < x < 0.0
            @error("x = $x is outside computed domain x <= $(x_list[end]).")
        elseif x >= 0.0
            @error("x should be a negative real number.")
        end
...
function iterate_inf_out(xm, s, epsilon, tau, kappa, lambda, N = _TruncatioN, l = nothing, tol = _TOLERANCE, sfe = 0)
    use_sfe = sfe == 1
    if use_sfe
        nu_out, coeffs_inf, Psfe_out, Rout = sfe_out(s, epsilon, tau, kappa, lambda, N, l)
        x_min = - Rout
...
function iterate_zero_in(xm, s, epsilon, tau, kappa, lambda, N = _TruncatioN, tol = _TOLERANCE, lfe = 0)
    if lfe == 1
        ...
        Xin, Rin = lfe_in_zero(s, epsilon, tau, kappa, lambda, N)
        ...
        function lfe_P(x)
            ...
            elseif x < x_list[end]
                @error("x = $x is outside computed domain x >= $(x_list[end]).")
            elseif x >= 0.0
                @error("x = $x should be a negative real number.")
            end
        ...
        coeffs_zero = an_zero(s, epsilon, tau, kappa, lambda, N)
        ...
        while x_max > xm
            coeffs_ord = ordinary_point_coeffs(s, epsilon, tau, kappa, lambda, x_max, P, P_prime, N)
            Δx = min(convergence_radius_x0_left(x_max, s, epsilon, tau, kappa, lambda, coeffs_ord, tol), convergence_radius_x0_right(x_max, s, epsilon, tau, kappa, lambda, coeffs_ord, tol))
            if !(isfinite(Δx)) || Δx <= 0
                @error("Step size Δx invalid: $Δx")
            end
...
function iterate_zero_out(xm, s, epsilon, tau, kappa, lambda, N = _TruncatioN, tol = _TOLERANCE, lfe = 0)
    if lfe == 1
        ...
        Xout, Rout = lfe_out_zero(s, epsilon, tau, kappa, lambda, N)
        ...
        coeffs_zero = bn_zero(s, epsilon, tau, kappa, lambda, N)
        coeffs_list = Vector{typeof(coeffs_zero)}()
        x_list = Vector{Float64}()
